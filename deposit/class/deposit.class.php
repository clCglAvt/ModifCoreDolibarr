...
/**
 *	\file       htdocs/compta/paiement/deposit/class/deposit.class.php
 *	\ingroup    compta
 *	\brief      File with class to manage deposit delivery receipts
 */
...
/**
 *	Class to manage deposit delivery receipts
 */
class Deposit extends CommonObject
{
	...
	function fetch($id,$ref='')
	{
		...
		$sql = "SELECT bc.rowid, bc.datec, bc.fk_user_author, bc.fk_type_paiement, bc.amount, bc.ref, bc.statut, bc.nbcheque, bc.ref_ext";
		$sql.= ", bc.date_bordereau as date_bordereau, sum(b.amount) as TotalPaiement , tp.libelle , tp.code";
		$sql.= ", ba.label as account_label";
		$sql.= " , count(b.rowid) as TotalNb ";
		...
		$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_paiement as tp ON b.fk_type = tp.code";
		$sql.= " WHERE bc.entity = ".$conf->entity;
		if ($id)  $sql.= " AND bc.rowid = ".$id;
		if ($ref) $sql.= " AND bc.ref = '".$this->db->escape($ref)."'";
		$sql.= " GROUP BY bc.rowid, bc.datec, bc.fk_user_author, bc.fk_bank_account, bc.amount, bc.ref, bc.statut, bc.nbcheque, bc.ref_ext, bc.date_bordereau ";
		...
		if ($resql)
		{
			if ($obj = $this->db->fetch_object($resql) )
			{
				...
				$this->mode_paiementid  = $obj->fk_type_paiement;
				$this->paiementlib 	  = $obj->libelle;
				$this->paiementcode 	  = $obj->code;
				$this->account_label  = $obj->account_label;
				$this->author_id      = $obj->fk_user_author;
				$this->nbpaiement       = $obj->nbcheque;
				$this->statut         = $obj->statut;
				$this->ref_ext        = $obj->ref_ext;
				$this->TotalPaiement  = $obj->TotalPaiement;
				$this->TotalNb 		  = $obj->TotalNb;
	 ...
	 *	@param  int		$account_id 	Bank account for deposit receipt
	 *  @param  int		$limit          Limit ref of deposit to this
	 *  @param	array	$toRemise		array with deposit's paiements to remise
	 *	@return	int						<0 if KO, >0 if OK
	 */
	function create($user, $account_id, $mode_paiementid, $limit, $toRemise)
	{
		global $conf;
		...
		if ( $resql )
		{
			...
			{
				$this->errno = -1024;
				dol_syslog(get_class($this)."::Create Error read id ".$this->errno, LOG_ERR);
			}
			...
			if ($this->id > 0 && $this->errno == 0)
			{
				...
				{
					$this->errno = -1025;
					dol_syslog(get_class($this)."::Create Error update ".$this->errno, LOG_ERR);
				}
			}
			...
			if ($this->id > 0 && $this->errno == 0)
			{
				...
				{
					$this->errno = -1026;
					dol_syslog(get_class($this)."::Create Error ".$this->errno, LOG_ERR);
				}
			}
			...
			if ($this->id > 0 && $this->errno == 0)
			{
				foreach ($lines as $lineid)
				{
					...
					if($checkremise==true)
					{
						...
						{
							$this->errno = -18;
							dol_syslog(get_class($this)."::Create Error update bank ".$this->errno, LOG_ERR);
						}
					}
				}
			}
			...
			if ($this->id > 0 && $this->errno == 0)
			{
				...
				{
					$this->errno = -1027;
					dol_syslog(get_class($this)."::Create Error update amount ".$this->errno, LOG_ERR);
				}
			}
		}
		else
	...
	/**
	 *	Ajoute un ou des paiements à un bordereau
	 *
	 *	
	 *  @param	array	$toRemise		array with deposit's paiements to remise
	 *	@return	int						<0 if KO, >0 if OK
	 */
	function AjouteCheque($toRemise)
	{ 
		foreach ($toRemise as $linetoremise)
		{
			$sql = "UPDATE ".MAIN_DB_PREFIX."bank";
			$sql.= " SET fk_bordereau = ".$this->id;
			$sql.= " WHERE rowid = ".$linetoremise;

			dol_syslog(get_class($this)."::AjouteCheque", LOG_DEBUG);
			$resql = $this->db->query($sql);
			if (!$resql)
			{
				$this->errno = -18;
				dol_syslog(get_class($this)."::AjouteCheque Error update bank ".$this->errno, LOG_ERR);
			}
		}
		...
		{
			$this->errno = -1027;
			dol_syslog(get_class($this)."::Error update amount deposit ".$this->errno, LOG_ERR);
		}
		return $this->errno;
	...
	function delete($user='')
	{
		...
		if ( $resql )
		{
			...
			if ($num <> 1) {
				$this->errno = -2;
				dol_syslog(get_class($this)."::Delete Erreur Lecture ID ($this->errno)");
			}
			...
			if ( $this->errno === 0) {
			    ...
			    {
			        $this->errno = -1028;
				    dol_syslog(get_class($this)."::Delete ERREUR UPDATE ($this->errno)");
				}
			}
		}
		...
		{
			$this->db->rollback();
			dol_syslog(get_class($this)."::Delete ROLLBACK ($this->errno)");
		}
	...
	function validate($user)
	{
		...
		if ($this->errno == 0 && $numref)
		{
			...
			if ( $resql )
			{
				...
				{
					$this->errno = -1029;
					dol_syslog(get_class($this)."::Validate Error ".$this->errno, LOG_ERR);
				}
			}
			else
			{
		...
		{
			$this->db->rollback();
			dol_syslog(get_class($this)."::Validate ".$this->errno, LOG_ERR);
            return $this->errno;
		}
	}

	/**
	 *      Return next reference of deposit receipts not already used (or last reference)
	...
	function getNextNumRef($mode='next')
	{
		...
		if (! empty($conf->global->CHEQUERECEIPTS_ADDON))
		{
			 ...
			 */
			if ($mode != 'last' && !$numref) {
				dol_print_error($db,"Deposit::getNextNumRef ".$obj->error);
				return "";
			}
	...
	function load_board($user)
	{
		...
		if ($resql)
		{
			...
			$response->url=DOL_URL_ROOT.'/compta/paiement/deposit/index.php?leftmenu=checks&amp;mainmenu=bank';
	...
	function generatePdf($model, $outputlangs)
	{
		...
		dol_syslog(get_class($this)."::generatePdf model=".$model." id=".$this->id, LOG_DEBUG);
		...
		if (file_exists($dir.$file))
		{
			...
			dol_syslog(get_class($this)."::generatePdf", LOG_DEBUG);
			...
			$docmodel->nbcheque = $this->nbpaiement;
			...
			$result=$docmodel->write_file($this, $conf->banque->dir_output.'/checkdeposits', $this->ref, $outputlangs);
			if ($result > 0)
			{
	...
	function removeCheck($account_id)
	{
		...
		if ($this->id > 0)
		{
			...
			{
				$this->errno = -1032;
				dol_syslog(get_class($this)."::removeCheck ERREUR UPDATE ($this->errno)");
			}
		}
		return 0;
	}
	...
	function set_number($user, $ref)
	{
		if ($user->rights->banque->cheque)
		{
			...
			dol_syslog(get_class($this)."::set_number", LOG_DEBUG);
			$resql=$this->db->query($sql);
			if ($resql)
			{
				return 1;
	...
	function getNomUrl($withpicto=0,$option='')
	{
        ...
        $link = '<a href="'.DOL_URL_ROOT.'/compta/paiement/deposit/card.php?id='.$this->id.'" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
		$linkend='</a>';
	...
	/**
	 *  Return label of a mode de paiement
	 *
	 *  @param	int		$id     	Identifiant du mode de paiement
	 *  @param  string	$type		'code' pour récuperer le code, 'libelle' pour récupérer le libellé
	 *  @return string      		Libelle du statut, '' si erreur
	 */
	function getLibModePaiement ($id, $type)
	{
		...
		if ($resql)
		{
			...
			{
				$this->db->free($resql);
				if ($type == 'code') return($obj->code);
				elseif ($type == 'libelle') return($obj->libelle);			
			}
		}
		else
		{
