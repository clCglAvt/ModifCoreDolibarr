 ...
 * Modif 3/3/2014 - Propale commissionnement avec TVA
 ...
 */
//Modif CCA 3/2/14 - Nom modèle
class pdf_azur_comTVA extends ModelePDFPropales
//Fin Modif CCA
{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
					{
						...
						if ($vat_rate > 0) $pdf->MultiCell($this->posxup-$this->posxtva-0.8, 3, $vat_rate, 0, 'R');
						else $pdf->MultiCell($this->posxup-$this->posxtva-0.8, 3, '(*)', 0, 'R');
					}
...
//Modif CCA 3/3/2014 - Propale commissionnement
		// Mention propre au conditionnement
			$pdf->SetFont('','', $default_font_size - 2);
			$pdf->SetXY($this->marge_gauche, $posy);
			$pdf->MultiCell(120, 3, $outputlangs->transnoentities("VATCommPropalCompteMoniteur"), 0, 'L', 0);
			$posy=$pdf->GetY()+3;
//fin Modif CCA 3/3/2014 -
...
//Modif CCA 3/3/2014 - Propale commissionnement
				...
				$pdf->SetFont('','', $default_font_size - 2);
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol1, $tab2_hl, $outputlangs->transnoentities("VATCommTVA"), $useborder, 'L', 0);
