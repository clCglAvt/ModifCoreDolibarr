				...
				if (! is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
				{
					...
					if (! class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/',$filebis) || preg_match('/mod_/',$classname)) && substr($filebis, dol_strlen($filebis)-3, 3) == 'php')
					{
						...
						if ($module->isEnabled())
						{
							...
							print '</td>';
							if (empty($module->nameclasscheque)) $chequereceipts=new RemiseCheque($db);
							else  {
								$classdircheque =  $module->dirclasscheque;
								require_once DOL_DOCUMENT_ROOT.$classdircheque ;
								
								$classnamecheque = $module->nameclasscheque;
								$chequereceipts=new $classnamecheque($db);
							}
