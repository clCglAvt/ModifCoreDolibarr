...
require_once DOL_DOCUMENT_ROOT.'/compta/paiement/deposit/class/html.deposit.class.php';
...
$arrayfields=array(
    ...
    'b.fk_bordereau'=>array('label'=>$langs->trans("Dépôt"), 'checked'=>1),
...
if (GETPOST('button_removefilter_x','alpha') || GETPOST('button_removefilter.x','alpha') || GETPOST('button_removefilter','alpha')) // All tests are required to be compatible with all browsers
{
	...
	$search_thirdparty='';
	$search_num_releve='';
	if ($action <> 'reconcile') $search_conciliated=''; // le test permet de rester dans le travail de rapprochement au moment où l'on vide les filtres 
	$thirdparty='';
	$bordereau ='';
...
if (GETPOST('confirm_reconcile') && $user->rights->banque->consolidate)
{
		...
		if (! $error)
		{
			// header('Location: '.$_SERVER["PHP_SELF"].'?id='.$id.'&action=reconcile&account='.$id.'&search_conciliated=0');
			if ($conf->global->DEPOSIT_MULTISUBMIT) header('Location: '.$_SERVER["PHP_SELF"].'?id='.$id.'&action=reconcile&search_conciliated=0');	// To allow a new submit
				else header('Location: '.$_SERVER["PHP_SELF"].'?id='.$id);	// To avoid to submit twice and allow back
        exit;
    }	
}
...
$sqlselect = "SELECT b.rowid, b.dateo as do, b.datev as dv, b.amount, b.label, b.rappro as conciliated, b.num_releve, b.num_chq,";
$sqlselect.= " b.fk_account, b.fk_type,";
$sqlselect.= " ba.rowid as bankid, ba.ref as bankref,";
$sqlselect.= " bu.url_id,";
$sqlselect.= " s.nom, s.name_alias, s.client, s.fournisseur, s.code_client, s.code_fournisseur, s.code_compta, s.code_compta_fournisseur";
$sqlselect.= ", b.fk_bordereau, bc.amount as TotDeposit,  bc.nbcheque, bc.date_bordereau, bc.ref ";
$sqlselect.= ",(SELECT SUM(b1.amount) FROM ".MAIN_DB_PREFIX."bank as b1 WHERE b1.fk_bordereau = bc.rowid ) as TotPaiementDeposit";
$sqlselect.= ",(SELECT COUNT(b1.rowid) FROM ".MAIN_DB_PREFIX."bank as b1 WHERE b1.fk_bordereau = bc.rowid ) as NbPaiementDeposit";
// Add fields from extrafields
foreach ($extrafields->attribute_label as $key => $val) $sqlselect.=($extrafields->attribute_type[$key] != 'separate' ? ",ef.".$key.' as options_'.$key : '');
// Add fields from hooks
$parameters=array();
$reshook=$hookmanager->executeHooks('printFieldListSelect',$parameters);    // Note that $action and $object may have been modified by hook
$sqlselect.=$hookmanager->resPrint;
$sqlfrom= " FROM ";
if ($bid) $sqlfrom.= MAIN_DB_PREFIX."bank_class as l,";
$sqlfrom.= " ".MAIN_DB_PREFIX."bank_account as ba,";
$sqlfrom.= " ".MAIN_DB_PREFIX."bank as b";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sqlfrom.= " LEFT JOIN ".MAIN_DB_PREFIX."bank_extrafields as ef on (b.rowid = ef.fk_object)";
$sqlfrom.= " LEFT JOIN ".MAIN_DB_PREFIX."bank_url as bu ON bu.fk_bank = b.rowid AND type = 'company'";
$sqlfrom.= " LEFT JOIN ".MAIN_DB_PREFIX."societe as s ON bu.url_id = s.rowid";
$sqlfrom.= ' LEFT JOIN '.MAIN_DB_PREFIX.'bordereau_cheque as bc ON bc.rowid=b.fk_bordereau';
$sqlwhere= " WHERE b.fk_account = ba.rowid";
$sqlwhere.= " AND ba.entity IN (".getEntity('bank_account').")";
if ($account > 0) $sqlwhere.=" AND b.fk_account = ".$account;
// Search period criteria
if (dol_strlen($search_dt_start)>0) $sqlwhere .= " AND b.dateo >= '" . $db->idate($search_dt_start) . "'";
if (dol_strlen($search_dt_end)>0) $sqlwhere .= " AND b.dateo <= '" . $db->idate($search_dt_end) . "'";
// Search period criteria
if (dol_strlen($search_dv_start)>0) $sqlwhere .= " AND b.datev >= '" . $db->idate($search_dv_start) . "'";
if (dol_strlen($search_dv_end)>0) $sqlwhere .= " AND b.datev <= '" . $db->idate($search_dv_end) . "'";
if ($search_ref) $sqlwhere.=natural_search("b.rowid", $search_ref);
if ($search_req_nb) $sqlwhere.= natural_search("b.num_chq", $search_req_nb);
if ($search_num_releve) $sqlwhere.= natural_search("b.num_releve", $search_num_releve);
if ($search_conciliated != '' && $search_conciliated != '-1') $sqlwhere.= " AND b.rappro = ".$search_conciliated;
if ($search_thirdparty) $sqlwhere.= natural_search("s.nom", $search_thirdparty);
if ($description) $sqlwhere.= natural_search("b.label", $description);       // Warning some text are just translation keys, not translated strings
if ($bid) $sqlwhere.= " AND b.rowid=l.lineid AND l.fk_categ=".$bid;
if (! empty($type)) $sqlwhere.= " AND b.fk_type = '".$db->escape($type)."' ";
// Search criteria amount
$debit = price2num(str_replace('-','',$debit));
$credit = price2num(str_replace('-','',$credit));
if ($debit) $sqlwhere.= natural_search('- b.amount', $debit, 1);
if ($credit) $sqlwhere.= natural_search('b.amount', $credit, 1);
// Add where from extra fields
if ($bordereau) $sqlwhere.= " AND b.fk_bordereau=".$bordereau;
foreach ($search_array_options as $key => $val)
{
    $crit=$val;
    $tmpkey=preg_replace('/search_options_/','',$key);
    ...
    {
        $sqlwhere .= natural_search('ef.'.$tmpkey, $crit, $mode);
    }
}
...
if ($resql)
{
	...
	print '<input type="hidden" name="search_conciliated" value="'.$search_conciliated.'">';
...
// Cellule pour liste des relevé, saisie numéro et le bouton conciliation
...
// Fin Cellule pour liste des relevé, saisie numéro et le bouton conciliation
	print '</td>';

	if ($affichedepot)
	{			
	// Cellule pour info bordereau si conciliation
		print '<td >';
		...
		$objp = $db->fetch_object($resql);
			// Bordereau de dépot
				...
				print '<tr><td> <b><i>'.$langs->trans( "BcMontant").'</i></b></td><td align=right>'.price($objp->TotDeposit).'</td></tr>';
				if ($objp->TotDeposit <> $objp->TotPaiementDeposit) 
				print '<tr><td style="color:red"> <b><i>'.$langs->trans( "PaiemtBdMontant").'</i></b></td><td  style="color:red" align=right>'.price($objp->TotPaiementDeposit).'</td></tr>';
				print '<tr><td> <b><i>'.$langs->trans( "NumberOfPaiemts").'</i></b></td><td align=right>'.$objp->nbcheque.'</td></tr>';
				if ($objp->nbcheque <> $objp->NbPaiementDeposit) 
				print '<tr><td style="color:red"> <b><i>'.$langs->trans( "NumberOfPaiemtsBc").'</i></b></td><td  style="color:red" align=right>'.price($objp->NbPaiementDeposit).'</td></tr>';
...
// Fin Cellule pour info bordereau si conciliation
...
//	    print '</td></tr></table>';
	...
	{
	    print '<td class="liste_titre">';    	
		$frmDeposit = new FormDeposit($db);
		$frmDeposit->select_depots(empty($bordereau)?'':$bordereau, 'bordereau',$sqlfrom, $sqlwhere,  1,'');
	...
	// Fields title
	$param.="&account=".$acct->id.'&bordereau='.$bordereau;
	print '<tr class="liste_titre">';
	...
	if (! empty($arrayfields['b.fk_bordereau']['checked']))  print_liste_field_titre($langs->trans("Dépôt"),$_SERVER["PHP_SELF"],"bc.ref","",$param,'align="left"',$sortfield,$sortorder);
	...
	// calcul du nb collone affichées avant la check_box de relevé
	$nbdisplaycol = 0;
	$displayfield = array();
	foreach($arrayfields as $key => $displayfield)
	{
		if ($key == 'b.num_releve') break;
		$nbdisplaycol += $displayfield["checked"];	
	}
	if (! empty($arrayfields['b.num_releve']['checked']))    {
			print '<tr><td ></td><td>';
			if ($conf->use_javascript_ajax) print '<tr><td colspan='.$nbdisplaycol.'></td><td align=right><a href="#AncreLstDetail" id="checkall_'.$bid.'">'.$langs->trans("All").'</a> / <a href="#AncreLstDetail" id="checknone_'.$bid.'">'.$langs->trans("None").'</a></td></tr>';
			print '</td></tr>';
	}
    ...
    {
        if (! $affichedepot or ( $affichedepot and $i>0) )$objp = $db->fetch_object($resql);
    	...
    	if (! empty($arrayfields['b.fk_bordereau']['checked'])) 
		{
    	   print '<td align="center" class="nowrap">'.$objp->ref."</td>\n";
                if (! $i) $totalarray['nbfield']++;
    	}
